...
/// <summary>
/// MySQL 8.0 平台库方言（B3b）：TOP→LIMIT（移至句尾）、ROW_NUMBER 分页→LIMIT 偏移、
/// DATEADD/DATEDIFF 桶→DATE_FORMAT 截断（列存 UTC 墙钟值，直接截断语义与 SQL Server 一致，
/// 不走 UNIX_TIMESTAMP——会话时区非 UTC 时日桶会偏移）、DELETE TOP→DELETE LIMIT、
/// @@ROWCOUNT→ROW_COUNT()、ESCAPE 反斜杠须字面量转义 '\\'、[Count]→反引号。
/// 真机验证并入 B9；ROW_NUMBER/PARTITION、COUNT(DISTINCT CASE)、窗口嵌套聚合 8.0 均原生支持。
/// </summary>
public sealed class MySqlDialect : IPlatformDialect
{
    ...
    {
        "minute" => $"CAST(DATE_FORMAT({column}, '%Y-%m-%d %H:%i:00') AS DATETIME(3))",
        "hour"   => $"CAST(DATE_FORMAT({column}, '%Y-%m-%d %H:00:00') AS DATETIME(3))",
        "day"    => $"CAST(DATE_FORMAT({column}, '%Y-%m-%d 00:00:00') AS DATETIME(3))",
    ...
    public string SlowSqlTemplatesSql(string where) => $"""
        ...
        SELECT fingerprint                                                     AS Fingerprint,
               ...
               AVG(CAST(duration_ms AS SIGNED))                                AS AvgMs,
               MAX(duration_ms)                                                AS MaxMs,
               SUM(cpu_ms)                                                     AS CpuTotalMs,
               AVG(CAST(cpu_ms AS SIGNED))                                     AS CpuAvgMs,
    ...
    public string SlowSqlTemplatesFromTopSqlSql(string where) => $"""
        ...
        SELECT d.fingerprint                                                  AS Fingerprint,
               ...
               CAST(SUM(d.exec_count) AS SIGNED)                              AS Count,
               ...
               CAST(SUM(d.total_elapsed_ms) * 1.0
                    / NULLIF(SUM(d.exec_count), 0) AS SIGNED)                 AS AvgMs,       -- 均值=Σ耗时/Σ次数（累计窗口求和口径）
               MAX(d.max_elapsed_ms)                                          AS MaxMs,
               SUM(d.total_worker_ms)                                         AS CpuTotalMs,
               CAST(SUM(d.total_worker_ms) * 1.0
                    / NULLIF(SUM(d.exec_count), 0) AS SIGNED)                 AS CpuAvgMs,
    ...
    // 与 @@IDENTITY 自增回填同款多语句批：同一会话内 ROW_COUNT() 紧跟 DELETE 取受影响行数
    public string HousekeepingDeleteBatchSql(string table, string column, int batchSize) => $"""
        /* dbpilot */
        DELETE FROM {table} WHERE {column} < @cutoff LIMIT {batchSize};
        SELECT ROW_COUNT();
        """;
