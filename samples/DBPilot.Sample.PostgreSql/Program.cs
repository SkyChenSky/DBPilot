...
// CLI：dotnet run --project samples/DBPilot.Sample.PostgreSql -- --hash <密码>  → 输出 PBKDF2 哈希（写入 appsettings DBPilot:Auth:PasswordHash）
...
try
{
    Log.Information("DBPilot.Sample.PostgreSql 启动中…");

    var builder = WebApplication.CreateBuilder(args);

    // 组合根只写"本形态的硬口径"：平台库引擎 = PostgreSQL（枚举写死）。
    ...
    // 监控引擎零代码：本形态引用三个引擎包 → 可接入 SQL Server / MySQL / PostgreSQL 实例（四形态冒烟宿主）。
    builder.AddDBPilot(o => o.PlatformEngine = DbpilotEngine.PostgreSql);
...
{
    Log.Fatal(ex, "DBPilot.Sample.PostgreSql 启动失败");
    return 1;
}
finally
{
