...
// CLI：dotnet run --project samples/DBPilot.Sample.SqlServer -- --hash <密码>  → 输出 PBKDF2 哈希（写入 appsettings DBPilot:Auth:PasswordHash）
...
try
{
    Log.Information("DBPilot.Sample.SqlServer 启动中…");

    var builder = WebApplication.CreateBuilder(args);

    // 组合根只写"本形态的硬口径"：平台库引擎 = SQL Server（枚举写死）。
    ...
    // 监控引擎零代码：csproj 引用哪个引擎包（DBPilot.SqlServer/DBPilot.MySql）即自动注册该引擎全部 Provider。
...
{
    Log.Fatal(ex, "DBPilot.Sample.SqlServer 启动失败");
    return 1;
}
finally
{
